...
mod tests {
    ...
    fn parses_valid_toml_schema() {
        ...
        assert_eq!(manifest.categories[0].default_minutes, 60);

        assert_eq!(manifest.schedule.clone().unwrap().len(), 1);
        assert_eq!(manifest.schedule.clone().as_ref().unwrap()[0].days, 127);
    }
    ...
    fn applies_default_values_for_missing_fields() {
        ...
        assert_eq!(manifest.categories[0].default_minutes, 30);
        assert_eq!(manifest.categories[0].color, "blue");
    }

    #[test]
    fn parses_app_config_successfully() {
        let raw_toml = r#"
            [general]
