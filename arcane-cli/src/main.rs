...
mod tests {
    ...
    async fn test_cli_import() {
        ...
        let manifest_path = std::env::current_dir().unwrap().join("test_manifest.toml");
        ...
        let cats = arcane_core::db::category::list_categories(&pool)
            .await
            .unwrap();
        ...
        let slots = arcane_core::db::schedule::list_schedule_slots_detail(&pool)
            .await
            .unwrap();
...
"#;
        ...
        let exported = arcane_core::db::schedule::export_manifest(&pool)
            .await
            .unwrap();
    ...
    async fn test_cli_today() {
        ...
        // Call today_cmd
        today_cmd(&pool, "19:00", "Work").await;
        today_cmd(&pool, "21:00", "rest").await;

        // Verify that they are inserted in the database
        let today = chrono::Local::now().date_naive();
        let list = arcane_core::db::schedule::list_schedule_overrides(today, &pool)
            .await
            .unwrap();
